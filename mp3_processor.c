...
extern status_t (*track_exports[MAX_DOC_TYPES]) (void *, const void *, FILE *);
...
status_t process_mp3_files(int max_input_files, char * input_files[], setup_t * setup) 
{
	...
	for(i = 0; i < max_input_files; i++ )
	{
		...
		if((st = process_mp3_file(mp3_file, tracks)) != OK)
		{
			ADT_Vector_delete(&tracks);
			fclose(mp3_file);
			return st;	
  	...
  	if ((file_out = fopen(setup->output_file, "wt")) == NULL)
	{
...
status_t process_mp3_file(FILE * fi, ADT_Vector_t * tracks)
{
	status_t st;
	ADT_Track_t * track;

	if(fi == NULL || tracks == NULL)
		return ERROR_NULL_POINTER;

	if((st = ADT_Track_new(&track)) != OK)
		return st;

	if((st = ADT_Track_get_info_for_fields(fi, track)) != OK)
	{
